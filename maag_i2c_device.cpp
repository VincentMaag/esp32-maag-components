...
esp_err_t MaagI2CDevice::read(uint8_t *data_reg_, size_t data_reg_size_, uint8_t *in_data_, size_t in_data_size_)
{
    // check input and output data is valid
    if(data_reg_ == NULL || in_data_ == NULL || data_reg_size_ < 1 || in_data_size_ < 1){
    ...
    i2c_master_read(cmd, in_data_, in_data_size_, I2C_MASTER_LAST_NACK);
    i2c_master_stop(cmd);
    // start sequence
...
esp_err_t MaagI2CDevice::readClockStretched(uint8_t *data_reg_, size_t data_reg_size_, uint8_t *in_data_, size_t in_data_size_, int delay_ms_)
{
    // check input and output data is valid
    if(data_reg_ == NULL || in_data_ == NULL || data_reg_size_ < 1 || in_data_size_ < 1){
        ESP_LOGE(TAG, "Invalid buffer pointers or invalid size");
    ...
    esp_err_t res = i2c_master_cmd_begin(m_port, cmd, I2CDEV_TIMEOUT / portTICK_PERIOD_MS);
    if (res != ESP_OK)
        ESP_LOGE(TAG, "Could not send cmd to device [0x%02x at %d]: %d", m_dev_addr, m_port, res);
    i2c_cmd_link_delete(cmd);

    //ESP_LOGI(TAG, "Sent cmd to device [0x%02x at %d]: %d", m_dev_addr, m_port, res);

    //ESP_LOGI(TAG, "pausing shortly...");
    
    // delay for 100ms between write and read calls, as the sensor can take
    // some time to respond.
    vTaskDelay(delay_ms_ / portTICK_RATE_MS);

    // get response data after a pause
    ...
    i2c_master_stop(cmd);
    // start sequence
    res = i2c_master_cmd_begin(m_port, cmd, I2CDEV_TIMEOUT / portTICK_PERIOD_MS);
    if (res != ESP_OK)
        ESP_LOGE(TAG, "Could not read from device [0x%02x at %d]: %d", m_dev_addr, m_port, res);
    i2c_cmd_link_delete(cmd);
